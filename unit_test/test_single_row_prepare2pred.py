...
import torch
import json
from melloddy_tuner.utils.single_row_prep2pred import SingleRowPreparator, KeyProviderTrivial, KeyProviderFromEnv, KeyProviderFromJsonFile
...
class Prepare2PredTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.key_json_file = os.path.join(curDir, "reference_files", "example_key.json")
        os.environ["MELLODDY_KEY"] =  "melloddy" #trivial key for testing
        self.key_provider_env = KeyProviderFromEnv("MELLODDY_KEY")
        self.key_provider_trivial = KeyProviderTrivial()
        self.key_provider_file = KeyProviderFromJsonFile(self.key_json_file)
        
    def test_key_providers(self):    
        self.assertEqual(self.key_provider_file.get_key(), self.key_provider_trivial.get_key())
        self.assertEqual(self.key_provider_env.get_key(), self.key_provider_trivial.get_key())

    def test_single_to_coo(self):
        my_srprep = SingleRowPreparator(key_provider = self.key_provider_env, params = self.config)
